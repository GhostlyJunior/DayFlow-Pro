const firebaseConfig = {
    ...
    Object.assign(LANGS.es,{no_tracked_time:"Aún no hay tiempo registrado esta semana"});
    Object.assign(LANGS.en,{no_tracked_time:"No time has been tracked this week yet"});
    Object.assign(LANGS.pt,{no_tracked_time:"Ainda não há tempo registrado nesta semana"});
    ...
    function renderDashboard(){
      ...
      const days=["L","M","X","J","V","S","D"], max=Math.max(1,...S.weekHrs), hasWeekData=S.weekHrs.some(h=>h>0);
      const weekBars=S.weekHrs.map((h,i)=>`<button class="bar ${h>0?"":"zero"}" type="button" data-tip="${h} ${t("hours")}" title="${h} ${t("hours")}" style="height:${h>0?Math.max(5,h/max*100):0}%"><span>${days[i]}</span></button>`).join("");
      const emptyWeek=hasWeekData?"":`<p class="bars-empty">${t("no_tracked_time")}</p>`;
      ["#bars","#statsBars"].forEach(sel=>{$(sel).classList.toggle("is-empty",!hasWeekData);$(sel).innerHTML=emptyWeek+weekBars});
